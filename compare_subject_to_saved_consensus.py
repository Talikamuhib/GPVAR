...
def compare_to_consensus(subject_matrix, consensus_matrix, sparsity=0.15):
    ...
    if n_edges > 0:
        thresh_subj = np.sort(vec_subj)[::-1][min(n_edges, len(vec_subj)-1)]
        thresh_cons = np.sort(vec_cons)[::-1][min(n_edges, len(vec_cons)-1)]
    ...
    # Cosine similarity
    norm_subj = np.linalg.norm(vec_subj)
    norm_cons = np.linalg.norm(vec_cons)
    if norm_subj > 0 and norm_cons > 0:
        cos_sim = np.dot(vec_subj, vec_cons) / (norm_subj * norm_cons)
    else:
        cos_sim = 0
...
def main():
    ...
    if consensus_path.exists():
        ...
        print(f"  • Non-zero edges: {np.sum(consensus_values > 0)} / {len(consensus_values)}")
        print(f"  • Mean edge weight: {np.mean(consensus_values):.4f}")
        print(f"  • Max edge weight: {np.max(consensus_values):.4f}")
            ...
            if Path(filepath).exists():
                ...
                if data is not None:
                    ...
                    if corr_matrix.shape[0] == n_channels:
                        ...
                        logger.warning(f"Dimension mismatch for {filepath}: {corr_matrix.shape} vs {n_channels}")
    ...
    if len(all_matrices) == 0:
        ...
        for i in range(n_ad):
            ...
            subj = consensus_matrix + ad_mod + noise
        ...
        for i in range(n_hc):
            noise = np.random.randn(n_channels, n_channels) * 0.08
            noise = (noise + noise.T) / 2
            subj = consensus_matrix + hc_mod + noise
            np.fill_diagonal(subj, 0)
            subj = np.clip(subj, 0, 1)
            all_matrices.append(subj)
    ...
    # =========================================================================
    # STEP 3: COMPARE EACH SUBJECT TO CONSENSUS
    # =========================================================================
    print("\n" + "="*70)
    print("STEP 3: COMPARING EACH SUBJECT TO SAVED CONSENSUS")
    print("="*70)
    
    results_list = []
    
    for i, (matrix, subj_id, group) in enumerate(zip(all_matrices, subject_ids, group_labels)):
        comparison = compare_to_consensus(matrix, consensus_matrix)
    ...
    print(f"""
┌────────────────────────────────────────────────────────────────────────────┐
│         SUMMARY: SUBJECT SIMILARITY TO SAVED CONSENSUS MATRIX              │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│  CONSENSUS MATRIX:                                                         │
│    Path: {CONSENSUS_MATRIX_PATH[:60]}...
│    Shape: {consensus_matrix.shape}                                                     │
│                                                                            │
...
│    Pearson r:  t = {t_stat_r:>7.3f}, p = {p_val_r:.4f}  {'***' if p_val_r < 0.001 else '**' if p_val_r < 0.01 else '*' if p_val_r < 0.05 else 'ns':>4}                       │
│    Jaccard:    t = {t_stat_j:>7.3f}, p = {p_val_j:.4f}  {'***' if p_val_j < 0.001 else '**' if p_val_j < 0.01 else '*' if p_val_j < 0.05 else 'ns':>4}                       │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
""")
    ...
    ax1.set_title('Saved Consensus Matrix\n(Distance-Dependent)', fontweight='bold', fontsize=11)
    ...
    sig_str = '***' if p_val_r < 0.001 else '**' if p_val_r < 0.01 else '*' if p_val_r < 0.05 else 'ns'
    ax2.set_title(f'Pearson r by Group\n(p = {p_val_r:.4f} {sig_str})', fontweight='bold', fontsize=11)
    ...
    ax3.set_title(f'Jaccard by Group\n(p = {p_val_j:.4f} {sig_str_j})', fontweight='bold', fontsize=11)
    ...
    ax4.set_title('Pearson r vs Jaccard', fontweight='bold', fontsize=11)
    ...
    ax6.set_title('All Subjects Ranked', fontweight='bold', fontsize=11)
    ...
    summary_text = f"""
    ══════════════════════════════════════════════════════════════════════════
                 EACH SUBJECT vs SAVED CONSENSUS MATRIX: RESULTS
    ══════════════════════════════════════════════════════════════════════════
    
    CONSENSUS MATRIX
    ────────────────────────────────────────────────────────────────────────────
    Path:              {CONSENSUS_MATRIX_PATH}
    Shape:             {consensus_matrix.shape}
    Non-zero edges:    {np.sum(consensus_matrix[np.triu_indices(n_channels, k=1)] > 0)}
    ...
    PEARSON r (Subject vs Saved Consensus)
    ────────────────────────────────────────────────────────────────────────────
    ...
    t-test (AD vs HC): t = {t_stat_r:.3f}, p = {p_val_r:.4f}  {sig_str}
    ...
    t-test (AD vs HC): t = {t_stat_j:.3f}, p = {p_val_j:.4f}  {sig_str_j}
    ...
    plt.suptitle('Each Subject vs Saved Consensus Matrix',
    ...
    # =========================================================================
    # STEP 7: SAVE ANALYSIS INFO
    # =========================================================================
    print("\n" + "="*70)
    print("STEP 7: SAVING ANALYSIS INFO")
    print("="*70)
    
    info_filename = output_folder / 'analysis_info.txt'
    with open(info_filename, 'w') as f:
        f.write("="*70 + "\n")
        f.write("SUBJECT vs SAVED CONSENSUS MATRIX ANALYSIS\n")
        f.write("="*70 + "\n\n")
        f.write(f"Analysis Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write(f"INPUT CONSENSUS MATRIX:\n")
        f.write(f"  Path: {CONSENSUS_MATRIX_PATH}\n")
        f.write(f"  Shape: {consensus_matrix.shape}\n\n")
        f.write(f"SUBJECTS:\n")
        f.write(f"  Total: {n_subjects}\n")
        f.write(f"  AD: {n_ad}\n")
        f.write(f"  HC: {n_hc}\n")
        f.write(f"  Channels: {n_channels}\n\n")
        f.write(f"OUTPUT FILES:\n")
        f.write(f"  consensus_matrix.npy - Copy of consensus matrix used\n")
        f.write(f"  subject_vs_saved_consensus.csv - Results table\n")
        f.write(f"  subject_vs_saved_consensus.png - Visualization\n")
        f.write(f"  analysis_info.txt - This file\n\n")
        f.write(f"SUMMARY STATISTICS:\n")
        f.write(f"  Pearson r (all): {df_results['Pearson_r'].mean():.4f} ± {df_results['Pearson_r'].std():.4f}\n")
        f.write(f"  Pearson r (AD):  {ad_results['Pearson_r'].mean():.4f} ± {ad_results['Pearson_r'].std():.4f}\n")
        f.write(f"  Pearson r (HC):  {hc_results['Pearson_r'].mean():.4f} ± {hc_results['Pearson_r'].std():.4f}\n")
        f.write(f"  t-test: t={t_stat_r:.4f}, p={p_val_r:.6f}\n")
    
    print(f"✓ Analysis info saved: {info_filename}")
...
OUTPUT FILES:
  ✓ consensus_matrix.npy - Copy of consensus matrix used
  ✓ subject_vs_saved_consensus.csv - All subject similarity metrics
  ✓ subject_vs_saved_consensus.png - Visualization figure
  ✓ analysis_info.txt - Analysis metadata

CONSENSUS MATRIX USED:
  {CONSENSUS_MATRIX_PATH}

CSV COLUMNS:
  • Subject_ID: Subject identifier
  • Group: AD or HC
  • Pearson_r: Correlation with saved consensus
